    ...
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'DM Sans',sans-serif", padding:"40px 20px" }}>
      <div style={{ maxWidth:600, margin:"0 auto" }}>
        <div style={{ textAlign:"center", marginBottom:40 }}>
          <div style={{ color:C.cyan, fontSize:28, fontWeight:800, letterSpacing:"-1px", marginBottom:6 }}>QuickInvest</div>
          <div style={{ color:C.text, fontSize:20, fontWeight:700, marginBottom:6 }}>Definí tu perfil de inversor</div>
          <div style={{ color:C.muted, fontSize:13, lineHeight:1.6 }}>
            Este cuestionario nos ayuda a conocerte mejor para darte recomendaciones personalizadas.<br/>
            Solo toma 2 minutos y podés cambiarlo cuando quieras.
          </div>
        </div>
        ...
        {!submitted ? (
          <>
                  ...
                  {q.opts.map((opt,j) => {
                    ...
                    return (
                      <button key={j} onClick={() => setAnswers(p=>({...p,[i]:q.scores[j]}))} style={{
                        background:sel?C.cyanDim:"#090D15", border:`1px solid ${sel?C.cyan:C.border}`,
                        borderRadius:8, padding:"9px 13px", color:sel?C.cyan:C.text,
                        cursor:"pointer", fontSize:12, textAlign:"left", fontFamily:"inherit", fontWeight:sel?600:400,
                      }}>{opt}</button>
                    );
                  })}
                </div>
            ...
            <button
              style={{ width:"100%", padding:14, background:C.cyan, color:"#07090F", border:"none", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer", fontFamily:"inherit", opacity:Object.keys(answers).length===QUESTIONS.length?1:0.4 }}
          ...
          <div style={{ background:C.card, border:`2px solid ${riskProfile.color}`, borderRadius:16, padding:32 }}>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={() => { setSubmitted(false); setAnswers({}); }} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.muted, borderRadius:8, padding:"10px 20px", cursor:"pointer", fontFamily:"inherit", fontWeight:600, fontSize:13 }}>
                Rehacer
              </button>
              <button onClick={handleSave} disabled={saving} style={{ flex:1, background:C.cyan, color:"#07090F", border:"none", borderRadius:8, padding:"10px 20px", cursor:"pointer", fontFamily:"inherit", fontWeight:700, fontSize:14, opacity:saving?0.7:1 }}>
                {saving ? "Guardando..." : "Confirmar mi perfil →"}
              </button>
            </div>
          </div>
        )}
